...
public class MajorChangeController {
	...
	public String gethumanfiles(Model model, HttpServletRequest request, MajorChange mChange) {
		...
		if ("query".equals(opreate)) {
			...
			getInfo(mChange);
			 ...
			 mChange.setNew_salary_sum(4599.0f);
	...
	public void getInfo(MajorChange majorChange) {
		...
		Integer salary_standard_id = Integer.parseInt(salary_standard_name.substring(0, salary_standard_name.lastIndexOf("/")));
		...
		// 拆分新一级
				...
				Integer new_salary_standard_id = Integer.parseInt(new_salary_standard_name.substring(0, new_salary_standard_name.lastIndexOf("/")));
